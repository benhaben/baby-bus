		...
		if ($.validator) {
			$('.login-form').validate({
				...
				submitHandler: function (form) {
				    ...
				    Utilities.Login(postData).done(function (data) {
				        if (data.Attach != null) {
				            $.cookie('Token', data.Attach.Token.Id, { expires: 1, path: '/' });
				            $.cookie('UserId', data.Attach.UserInfo.UserId, { expires: 1, path: '/' });
				            $.cookie('UserGuid', data.Attach.UserInfo.Guid, { expires: 1, path: '/' });
				            $.cookie('Role', data.Attach.UserInfo.Role, { expires: 1, path: '/' });
				            $.cookie('KindergartenId', data.Attach.UserInfo.KindergartenId, { expires: 1, path: '/' });
				            $.cookie('ClassId', data.Attach.UserInfo.ClassId, { expires: 1, path: '/' });
				            $.cookie('ClassName', data.Attach.UserInfo.ClassName, { expires: 1, path: '/' });
				            $.cookie('KindergartenName', data.Attach.UserInfo.KindergartenName, { expires: 1, path: '/' });
				            $.cookie('LoginUserName', data.Attach.UserInfo.UserName, { expires: 1, path: '/' });

				            Utilities.Redirect("statistics.html");
				        } else {
				            NProgress.start(); // Demo Purpose Only!
				            $('.login-form .alert-danger').text("user name or password error!");
				            $('.login-form .alert-danger').show();
				            NProgress.done(); // Demo Purpose Only!
				        }
				        ...
				        NProgress.start(); // Demo Purpose Only!
				        $('.login-form .alert-danger').text(data.toLocaleString());
				        $('.login-form .alert-danger').show();
				        NProgress.done(); // Demo Purpose Only!
				        $("#box").css({ "display": "none" });
				    });
