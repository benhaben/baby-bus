...
//            , string value = "", UITableViewCellStyle style = UITableViewCellStyle.Value1)
            : base(caption)
        {//                , value, style) 
            _summarySection = -1;
...
//            this.Style = UITableViewCellStyle.Value1;
        ...
        public void Prepare()
        {
//            yin fix bug: use mvx binding instead current
//            int current = 0;
            foreach (Section s in Sections)
            {
                foreach (Element e in s.Elements)
                {
                    ...
                    if (re != null)
//                        re.RadioIdx = current++;
                        re.RadioIdx = Convert.ToInt32(re.Value);
        ...
        public bool ContainsElement(Section s)
        {
...
#warning More needed here for two way binding!
        ...
        //        EventHandler1
        public  EventHandler RadioSelectedChanged;
...
//            }
            ...
            if (radio != null)
            {
//                this function is not suitable for mvx radio binding
//                , variable current is start with 0. but id is the key in the object
...
//                }
                ...
                foreach (var s in Sections)
                {
                    foreach (var e in s.Elements)
                    {
                        ...
                        //yin fix bugs: you should store id in the value by mvx bind
                        if (Convert.ToInt32(radioElement.Value) == selected && cell.DetailTextLabel != null)
                        {
                            //yin fix bugs: show caption instead of summary, summary is null. i don't understand what's the summary
                            cell.DetailTextLabel.Text = radioElement.Caption;     
                            goto le;
                        }
                    }
                }
