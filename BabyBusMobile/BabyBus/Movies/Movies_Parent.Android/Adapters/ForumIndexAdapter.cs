...
{
	public class ForumIndexAdapter : BaseAdapter
	{
		protected readonly MvxActivity activity;
		public List<ECPostInfo> list;
		protected ForumIndexViewModel indexVM;

		public string PicassoTag{ get; private set; }

		public ForumIndexAdapter(MvxActivity activity, List<ECPostInfo> list)
		{
			this.activity = activity;
			this.list = list;
			PicassoTag = "ForumIndexAdapterTag";
		}
			...
			if (ischanged) {
				...
				if (!string.IsNullOrEmpty(item.FirstImage)) {
					...
					Picasso.With(activity).Load(Constants.ThumbServerPath + item.FirstImage).Placeholder(Resource.Drawable.icon_course)
						.Resize(80, 80).Tag(PicassoTag).Into(holder.ImagePost);
				} else {
		...
		{
			indexVM = indexVM ?? ((ForumIndexView)activity).ViewModel;
			var positon = (int)((LinearLayout)sender).Tag;
			var item = list[positon];
			indexVM.ShowForumDetailViewModel(item.PostInfoId);
		}
