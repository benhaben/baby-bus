        ...
        @autoreleasepool {
            ...
            void (^assetGroupEnumberatorFailure)(NSError *) = ^(NSError *error) {
                ...
                if ([ALAssetsLibrary authorizationStatus] == ALAuthorizationStatusDenied) {
                    NSString *errorMessage = NSLocalizedString(@"这个应用程序不能访问您的照片或视频。你可以在隐私设置中开启权限。", nil);
                    [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"禁止访问。", nil) message:errorMessage delegate:nil cancelButtonTitle:NSLocalizedString(@"好", nil) otherButtonTitles:nil] show];

                } else {
                    NSString *errorMessage = [NSString stringWithFormat:@"相册发生错误: %@ - %@", [error localizedDescription], [error localizedRecoverySuggestion]];
                    [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"错误", nil) message:errorMessage delegate:nil cancelButtonTitle:NSLocalizedString(@"好", nil) otherButtonTitles:nil] show];
                }
