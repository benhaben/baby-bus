...
namespace BabyBus.Net.Communication
{
    ...
    {
        Task<ApiResponser> SendQuestion(QuestionModel model);
        Task<ApiResponser> SendAnswer(AnswerModel model);
        Task<ApiResult<QuestionModel>> GetNewQuestionList(User user, long maxId = 0);

        Task<ApiResult<QuestionModel>> GetOldQuestionList(User user, long minId);
    }
